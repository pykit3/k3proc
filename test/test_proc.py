...
class TestProc(unittest.TestCase):
    ...
    def test_procerror(self):
        ...
        self.assertEqual(ex_args, (ex.returncode, ex.stdout, ex.stderr, ex.out, ex.err, ex.cmd, ex.options))
    ...
    def test_error_str(self):
        try:
            k3proc.command(
                "python",
                "-c",
                'import sys, os; os.write(1, b"foo"); os.write(2, b"bar"); sys.exit(1)',
                check=True,
                env={"foo": "bar"},
                cwd="/tmp",
                input="123",
        ...
        try:
            k3proc.command(
                "python",
                "-c",
                'import sys, os; os.write(1, b"\x01"); os.write(2, b"\x02"); sys.exit(1)',
                check=True,
                env={"foo": "bar"},
                cwd="/tmp",
                text=False,
    ...
    def test_text_true(self):
        cmd = [
            "python",
            "-c",
            'import os; os.write(1, b"\\x89")',
        ]

        self.assertRaises(UnicodeDecodeError, k3proc.command, *cmd)
    ...
    def test_close_fds(self):
        ...
        with open(read_fd) as f:
            ...
            returncode, out, err = k3proc.command("python", read_fd, str(fd), close_fds=False)
            ...
            returncode, out, err = k3proc.command("python", read_fd, str(fd), close_fds=True)
    ...
    def test_env(self):
        returncode, out, err = k3proc.command("python", "print_env.py", "abc", env={"abc": "xyz"}, cwd=this_base)
        dd("returncode:", returncode)
        dd("out:", out)
        dd("err:", err)
    ...
    def test_timeout(self):
        with k3ut.Timer() as t:
            self.assertRaises(
                k3proc.TimeoutExpired, k3proc.command, "python", "-c", "import time; time.sleep(1)", timeout=0.1
            )
...
capture={}
    ...
    def test_shell_script(self):
        returncode, out, err = k3proc.shell_script("ls " + this_base + " | grep init | grep -v pyc")
